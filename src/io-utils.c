...
struct user_fmt dol =
{ "dollar", "$", "($", 0, ")", "$0", ",", ".", FLOAT_PREC, 1};

struct user_fmt cma =
{ "comma", 0, "(", 0, ")", "0", ",", ".", FLOAT_PREC, 1};

struct user_fmt pct =
#if 0
{ "percent", 0, "-", "%", "%", "0%", 0, ".", 3, 100};
#else
{ "percent", 0, "-", "%", "%", "0%", 0, ".", FLOAT_PREC, 100};
#endif

struct user_fmt fxt =
{ "fixed", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1};

/* Variables */

struct user_fmt u[16] =
{
  {"user1", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user2", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user3", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user4", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user5", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user6", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user7", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user8", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user9", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user10", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user11", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user12", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user13", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user14", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user15", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
  {"user16", 0, "-", 0, 0, "0", 0, ".", FLOAT_PREC, 1},
};
...
flt_to_str (double val)
{
  ...
  return pr_flt (val, &fxt, FLOAT_PREC);
}
  ...
  if (prec)
    {
      ...
      p1 = (prec == FLOAT_PREC) ? 15 : (prec > 0) ? prec : -prec;
      pf = fmt->decpt;
      while (pf && *pf)
	*fptr++ = *pf++;
      /* *fptr++='.'; */
  ...
  if (fract)
      ...
      if (tmpval > 4)
	{
	  ...
	  for (pptr = fptr - 1;; --pptr)
		...
		{
		  if (pptr == fptr - 1 && pptr > &print_buf[BIGFLT] && (prec < 0 || prec == FLOAT_PREC))
		    {
		      --fptr;
